...
const helper = new SelectorHelper(page, {
  defaultTimeout: 30000,
  highlightElement: true
});

await helper.click('//button[@id="submit"]');
await helper.type('//input[@name="username"]', 'myuser');
const text = await helper.getText('//div[@class="result"]');

// Example 2: CSS Selector
await helper.click('#submit-button');
await helper.type('input[name="email"]', 'test@example.com');

// Example 3: Text selector
await helper.click('text=Login');
await helper.hover('text:Submit Form');

// Example 4: XPath Builder
...
const items = await helper.extractList('//div[@class="item"]', {
  title: './/h2/text()',
  price: '@data-price',
  link: './/a/@href'
});
...
const table = await helper.extractTable('//table[@id="data-table"]');
console.log(table.headers);
console.log(table.rows);

// Example 7: Advanced XPath
const complexXPath = new XPathBuilder()
  .root()
  .tag('div')
  .containsAttr('class', 'product')
  .and()
...
const price = await helper.getText(complexXPath);

// Example 8: Conditional actions
if (await helper.exists('//div[@class="error"]')) {
  const errorMsg = await helper.getText('//div[@class="error"]');
  console.log('Error:', errorMsg);
}

// Example 9: Wait patterns
await helper.waitFor('//div[@id="loading"]');
await helper.waitForDisappear('//div[@id="loading"]');

// Example 10: Multi-step interaction
await helper.click('//button[@id="open-modal"]');
await helper.waitFor('//div[@class="modal"]');
await helper.type('//input[@name="search"]', 'product name');
await helper.click('//button[@type="submit"]');
await helper.waitForDisappear('//div[@class="loading"]');
const results = await helper.extractList('//div[@class="result-item"]', {
  name: './/h3',
  description: './/p[@class="desc"]'
});

// Example 11: File upload
await helper.uploadFile('//input[@type="file"]', './document.pdf');

// Example 12: Drag and drop
await helper.dragAndDrop(
  '//div[@id="item-1"]',
  '//div[@id="drop-zone"]'
);

// Example 13: Custom evaluation
const customData = await helper.evaluate('//div[@id="data"]', (el) => {
  return {
    width: el.offsetWidth,
    height: el.offsetHeight,
    customAttr: el.dataset.custom
  };
});

// Example 14: Checkbox handling
await helper.check('//input[@type="checkbox"][@name="agree"]', true);
const isChecked = await helper.isChecked('//input[@name="agree"]');

// Example 15: Select dropdown
await helper.select('//select[@name="country"]', 'Vietnam');

// Example 16: Screenshot element
const imageBuffer = await helper.screenshot('//div[@class="chart"]', {
  screenshotOptions: { path: 'chart.png' }
});

// Example 17: Complex XPath with multiple conditions
const xpath = new XPathBuilder()
  .root()
  .tag('tr')
  .containsAttr('class', 'data-row')
  .descendant('td')
  .index(3)
  .build();
// Result: //tr[contains(@class, 'data-row')]//td[3]

// Example 18: Navigation through siblings
const siblingXPath = new XPathBuilder()
  .root()
  .tag('div')
  .id('current')
  ...
  .first()
  .build();
// Result: //div[@id='current']/following-sibling::div[1]

// Example 19: Parent navigation
const parentXPath = new XPathBuilder()
  .root()
  .tag('span')
  .text('Price:')
  .parent()
  ...
  .class('value')
  .build();
// Result: //span[contains(text(), 'Price:')]/../span[contains(@class, 'value')]

// Example 20: Multiple attributes with OR
const orXPath = new XPathBuilder()
  .root()
  .tag('button')
  .attr('type', 'submit')
  .or()
  .attr('type', 'button')
  .build();
// Result: //button[@type='submit' or @type='button']
*/